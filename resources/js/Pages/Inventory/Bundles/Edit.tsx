...
import type { Article, Bundle } from '@/types';
...
export default function EditBundle({ bundle, articles }: EditBundleProps) {
    const { t } = useTranslation();

    const initialItems: BundleFormItem[] = (bundle.bundle_items || []).map((bi) => ({
        article_id: String(bi.article_id),
    ...
    const { data, setData, put, processing, errors } = useForm<{
        ...
        items: initialItems.length > 0 ? initialItems : [{ article_id: '', quantity: 1 }],
    });
    ...
    return (
        <AppLayout>
            <Head title={t('inventory.edit_bundle')} />
            ...
            <div className="max-w-2xl mx-auto">
                <div className="mb-6">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">{t('inventory.edit_bundle')}</h1>
                    <p className="mt-1 text-sm text-gray-500">{t('inventory.edit_bundle_subtitle')}</p>
                </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div>
                                <Label htmlFor="is_active">{t('inventory.status')}</Label>
                                <Select
                                    value={data.is_active ? 'active' : 'inactive'}
                                    onValueChange={(val) => setData('is_active', val === 'active')}
                                >
