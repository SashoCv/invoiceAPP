...
export default function EditGoodsIssue({ issue, articles, clients, movements }: Props) {
    ...
    const { data, setData, errors, setError } = useForm({
        date: issue.date,
        notes: issue.notes || '',
        client_id: issue.client_id ? String(issue.client_id) : '',
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        setSubmitting(true);
        router.put(`/goods-issues/${issue.id}`, {
            date: data.date,
            notes: data.notes,
    ...
    return (
        <AppLayout>
            <Head title={`${t('inventory.edit_goods_issue')} ${issue.issue_number}`} />

            <div className="max-w-4xl mx-auto">
                <div className="mb-6">
                    <Link
                        href={`/goods-issues/${issue.id}`}
                        className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700"
                    >
                        <ArrowLeft className="w-4 h-4 mr-1" />
                        {t('inventory.back_to_issue')}
                    </Link>
                </div>
                    ...
                    <Card className="mb-6">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                ...
                                {t('inventory.edit_goods_issue')} — {issue.issue_number}
                            </CardTitle>
                            <CardDescription>{t('inventory.edit_goods_issue_subtitle')}</CardDescription>
                        </CardHeader>
