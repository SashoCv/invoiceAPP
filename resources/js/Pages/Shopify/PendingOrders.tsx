...
export default function PendingOrders({ orders, filters }: Props) {
    ...
    const applyFilters = () => {
        router.get('/shopify/orders/pending', {
            ...(from && { from }),
            ...(to && { to }),
        }, { preserveState: true });
    };
    ...
    return (
        <AppLayout>
            <Head title={t('shopify.pending_orders_title')} />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            ...
                            {t('shopify.pending_orders_title')}
                        </h1>
                        <p className="text-sm text-gray-500 mt-1">{t('shopify.pending_orders_subtitle')}</p>
                    </div>
                </div>
                ...
                {orders.data.length === 0 ? (
                    <Card>
                        <CardContent className="flex flex-col items-center justify-center py-16">
                            ...
                            <h3 className="text-lg font-medium text-gray-900">{t('shopify.no_pending_orders')}</h3>
                            <p className="text-sm text-gray-500 mt-1">{t('shopify.no_pending_orders_description')}</p>
                        </CardContent>
                    </Card>
                ) : (
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('shopify.pending_orders_title')} ({orders.total})</CardTitle>
                        </CardHeader>
