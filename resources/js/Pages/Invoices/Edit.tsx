...
import type { Invoice, Client, Article, Bundle } from '@/types';
...
export default function EditInvoice({ invoice, clients, articles, bundles = [] }: EditInvoiceProps) {
    ...
    return (
        <AppLayout>
            <Head title={`${t('invoices.edit_title')} - ${invoice.invoice_number}`} />
                ...
                <div className="mb-6">
                    <Button variant="ghost" size="sm" asChild className="mb-4">
                        <Link href={`/invoices/${invoice.id}`} className="flex items-center gap-2">
                            ...
                            {t('invoices.back_to_invoice')}
                        </Link>
                    </Button>
                    <h1 className="text-2xl font-bold text-gray-900">{t('invoices.edit_title')}</h1>
                ...
                <form onSubmit={submit}>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select value={data.status} onValueChange={(v: Invoice['status']) => setData('status', v)}>
                                ...
                                {data.items.map((item, index) => (
                                    <div key={index} className="p-4 border rounded-lg bg-gray-50">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                                            ...
                                            <div>
                                                <Label>{t('invoices.additional_discount')}</Label>
                                                <Input
                                                    type="number"
                                                    step="0.01"
                                                    min="0"
                    ...
                    <div className="flex items-center justify-end gap-4">
                        <Button variant="outline" asChild>
                            <Link href={`/invoices/${invoice.id}`}>{t('general.cancel')}</Link>
                        </Button>
