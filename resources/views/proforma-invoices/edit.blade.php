<x-app-layout>
    <div x-data="proformaForm()" x-init="init()">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('proforma.edit_proforma') }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ $proformaInvoice->formatted_number }}</p>
            </div>
        </div>

        <form action="{{ route('proforma-invoices.update', $proformaInvoice) }}" method="POST" @submit="validateForm">
            @csrf
            @method('PUT')
            ...
            <div class="max-w-5xl mx-auto space-y-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="proforma_prefix" id="proforma_prefix"
                                       value="{{ old('proforma_prefix', $proformaInvoice->proforma_prefix) }}"
                                ...
                                <span class="text-gray-500 font-medium">{{ $proformaInvoice->proforma_year }}-</span>
                                <input type="number" name="proforma_sequence" id="proforma_sequence" required
                                       value="{{ old('proforma_sequence', $proformaInvoice->proforma_sequence) }}"
                                       min="1"
                        ...
                        <div>
                            ...
                            <select name="client_id" id="client_id" required
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client_id', $proformaInvoice->client_id) == $client->id ? 'selected' : '' }}>
                                        {{ $client->display_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="issue_date" id="issue_date" required
                                   value="{{ old('issue_date', $proformaInvoice->issue_date->format('Y-m-d')) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                            @error('issue_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="valid_until" id="valid_until" required
                                   value="{{ old('valid_until', $proformaInvoice->valid_until->format('Y-m-d')) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                            @error('valid_until')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-1">
                                {{ __('proforma.status') }} <span class="text-red-500">*</span>
                            </label>
                            <select name="status" id="status" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                                <option value="draft" {{ old('status', $proformaInvoice->status) === 'draft' ? 'selected' : '' }}>{{ __('proforma.status_draft') }}</option>
                                <option value="sent" {{ old('status', $proformaInvoice->status) === 'sent' ? 'selected' : '' }}>{{ __('proforma.status_sent') }}</option>
                                <option value="converted_to_invoice" {{ old('status', $proformaInvoice->status) === 'converted_to_invoice' ? 'selected' : '' }}>{{ __('proforma.status_converted') }}</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    ...
                    <textarea name="notes" rows="2"
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">{{ old('notes', $proformaInvoice->notes) }}</textarea>
                </div>

                <!-- Actions -->
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('proforma-invoices.show', $proformaInvoice) }}"
    ...
    <script>
        function proformaForm() {
            return {
                ...
                taxAmount: 0,
                total: 0,
                currency: '{{ $proformaInvoice->currency ?? "MKD" }}',
                currencySymbol: '{{ $proformaInvoice->currency_symbol ?? "ден." }}',
                currencySymbols: { 'MKD': 'ден.', 'EUR': '€', 'USD': '$' },
                articles: @json($articles),

                init() {
                    @php
                        $existingItems = $proformaInvoice->items->map(function($item) {
                            return [
                                'article_id' => '',
