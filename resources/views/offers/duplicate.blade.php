<x-app-layout>
    <div x-data="offerForm()" x-init="init()">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('offers.duplicate_offer') }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ __('offers.duplicate_from') }}: {{ $offer->formatted_number }}</p>
            </div>
        </div>

        <form action="{{ route('offers.store') }}" method="POST" @submit="validateForm">
            @csrf
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="offer_prefix" value="{{ old('offer_prefix', $offer->offer_prefix) }}"
                                ...
                                <input type="number" name="offer_sequence" required value="{{ old('offer_sequence', $nextSequence) }}"
                                       min="1"
                        ...
                        <div>
                            ...
                            <input type="date" name="issue_date" required value="{{ old('issue_date', date('Y-m-d')) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="valid_until" required value="{{ old('valid_until', date('Y-m-d', strtotime('+30 days'))) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" required value="{{ old('title', $offer->title) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
            ...
            return {
                ...
                init() {
                    ...
                    if (this.items.length === 0) {
                        this.items = [{ description: '', quantity: 1, unit_price: 0, tax_rate: 18 }];
                    }

                    this.calculateTotals();
                },

                updateCurrency() { this.currencySymbol = this.currencySymbols[this.currency] || this.currency; },
                ...
                getItemTotal(item) {
                    ...
                    return subtotal + subtotal * (parseFloat(item.tax_rate) || 0) / 100;
                },
                calculateTotals() {
                ...
                formatNumber(num) { return new Intl.NumberFormat('mk-MK', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num || 0); },
                validateForm(e) { if (this.hasItems && this.items.length === 0) { e.preventDefault(); return false; } return true; }
            }
        }
    </script>
    @endpush
