<x-app-layout>
    <div x-data="invoiceForm()" x-init="init()">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('invoices.edit_invoice') }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ $invoice->formatted_number }}</p>
            </div>
        </div>

        <form action="{{ route('invoices.update', $invoice) }}" method="POST" @submit="validateForm">
            @csrf
            @method('PUT')
            ...
            <div class="max-w-5xl mx-auto space-y-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="invoice_prefix" id="invoice_prefix"
                                       value="{{ old('invoice_prefix', $invoice->invoice_prefix) }}"
                                ...
                                <span class="text-gray-500 font-medium">{{ $invoice->invoice_year }}-</span>
                                <input type="number" name="invoice_sequence" id="invoice_sequence" required
                                       value="{{ old('invoice_sequence', $invoice->invoice_sequence) }}"
                                       min="1"
                        ...
                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-1">
                                {{ __('invoices.status') }} <span class="text-red-500">*</span>
                            </label>
                            <select name="status" id="status" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                                <option value="draft" {{ old('status', $invoice->status) === 'draft' ? 'selected' : '' }}>{{ __('invoices.status_draft') }}</option>
                                <option value="sent" {{ old('status', $invoice->status) === 'sent' ? 'selected' : '' }}>{{ __('invoices.status_sent') }}</option>
                                <option value="unpaid" {{ old('status', $invoice->status) === 'unpaid' ? 'selected' : '' }}>{{ __('invoices.status_unpaid') }}</option>
                                <option value="paid" {{ old('status', $invoice->status) === 'paid' ? 'selected' : '' }}>{{ __('invoices.status_paid') }}</option>
                                <option value="overdue" {{ old('status', $invoice->status) === 'overdue' ? 'selected' : '' }}>{{ __('invoices.status_overdue') }}</option>
                                <option value="cancelled" {{ old('status', $invoice->status) === 'cancelled' ? 'selected' : '' }}>{{ __('invoices.status_cancelled') }}</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    ...
                    <textarea name="notes" rows="2"
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">{{ old('notes', $invoice->notes) }}</textarea>
                </div>
    ...
    <script>
        function invoiceForm() {
            return {
                ...
                currencySymbol: '{{ $invoice->currency_symbol ?? "ден." }}',
