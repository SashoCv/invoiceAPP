...
class AccountingReportController extends Controller
{
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        [$tab, $from, $to, $type] = $this->params($request);
        $report = $this->build($request->user()->id, $tab, $from, $to, $type, false);
        $rows = $tab === 'stock' ? $this->stockCsvRows($report) : $this->documentCsvRows($tab, $report);
    ...
    private function params(Request $request): array
    {
        ...
        return [$tab, $from->toDateString(), $to->toDateString(), $tab === 'stock' ? null : $type];
    }
    ...
    {
        $name = ['inputs' => 'vlezni_kalkulacii', 'outputs' => 'izlezni_kalkulacii', 'stock' => 'lager'][$tab];
    ...
    {
        return $tab === 'stock'
            ? $this->buildStock($userId, $from, $to)
            : $this->buildDocuments($userId, $tab, $from, $to, $type, $withLines);
    }
    ...
    private function buildDocuments(int $userId, string $tab, string $from, string $to, ?string $type, bool $withLines): array
    {
        ...
        foreach ($this->valuation->eventsBetween($userId, $from, $to) as $e) {
            ...
            $d['cost_tax'] += round($e['cost_value'] * $e['tax_rate'] / 100, 2);
    ...
    private function finaliseRow(array $d, string $tab): array
    {
        ...
        // РУЦ only makes sense for sales documents
        $d['margin'] = in_array($d['type'], ['invoice', 'shopify'], true)
    ...
    private function articles(int $userId): array
    {
        return DB::table('articles')->where('user_id', $userId)
            ->get(['id', 'code', 'name', 'unit'])->keyBy('id')->all();
    }
    ...
    private function documentCsvRows(string $tab, array $report): array
    {
        ...
        $header = $isOut
            ...
            : ['Р.бр.', 'Тип', 'Број', 'Датум', 'Ставки', 'Количина', 'Набавна без ДДВ', 'ДДВ', 'Набавна со ДДВ', 'Проценета'];
        ...
        $line = function (array $r) use ($isOut, $n) {
            ...
            return $isOut
                ...
                : [$r['rb'], $r['type_label'], $r['number'], $date, $r['items'], $n($r['quantity']), $n($r['cost_value']), $n($r['cost_tax']), $n($r['cost_with_tax']), $r['estimated'] ? 'да' : ''];
        };
        ...
        $total = function (string $label, array $t) use ($isOut, $n) {
            return $isOut
                ...
                : ['', $label, '', '', '', $n($t['quantity']), $n($t['cost_value']), $n($t['cost_tax']), $n($t['cost_with_tax']), ''];
        };
    ...
    private function stockCsvRows(array $report): array
    {
        ...
        $rows[] = [];
        $rows[] = ['Лагер листа'];
        $rows[] = ['Шифра', 'Назив', 'Ед.', 'Количина', 'Просечна набавна цена', 'Набавна вредност'];
        foreach ($report['list'] as $r) {
            $rows[] = [$r['code'], $r['name'], $r['unit'], $n($r['quantity']), number_format((float) $r['avg_cost'], 4, '.', ''), $n($r['value'])];
        }
        $rows[] = ['', 'ВКУПНО', '', $n($report['list_totals']['quantity']), '', $n($report['list_totals']['value'])];
